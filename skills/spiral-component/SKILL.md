...
2. Open Storybook (`pnpm --filter @spiral/docs dev`) — confirm the component story renders with all variants.
3. Confirm no hardcoded colors (no hex/rgb) and no `lucide-react` imports in the component file.
4. Confirm the component is re-exported from `packages/ui/src/index.ts`.
5. If user-visible: confirm `changelogs/{DisplayName}.md` is updated and a changeset exists.
